...
HRESULT CImGui_Manager::ImGui_Initialize(ID3D11Device** ppDevice, ID3D11DeviceContext** ppContext,
										CLevel_Edit* pLevelEdit, ID3D11ShaderResourceView** ppSRV)
{
...
void CImGui_Manager::Draw_Viewport()
{
	...
	const _float4x4* pView = m_pGI_Proxy->Get_Matrix(D3DTS::VIEW, PROJ_TYPE::PERSPEC);
	const _float4x4* pProj = m_pGI_Proxy->Get_Matrix(D3DTS::PROJ, PROJ_TYPE::PERSPEC);
