...
PS_OUT PS_BODY(VS_OUT In)
{
    ...
    float fMask = g_BodyMaskTexture.Sample(LinearSampler, In.vTexcoord).r;
    return Make_GBuffer(In, vAlbedo * fMask, Nw, vMRA);
}
...
PS_OUT PS_SHELL(VS_OUT In)
{
    float3 vAlbedo, vMRA, Nw;

    if (In.vTexcoord1.x <= 1.f && In.vTexcoord1.y <= 1.f)
        Sample_PBRSet(g_DiffuseTexture1, g_MRATexture1, g_NormalTexture1, In.vTexcoord1, In, vAlbedo, vMRA, Nw);
    else
        Sample_PBRSet(g_DiffuseTexture, g_MRATexture, g_NormalTexture, In.vTexcoord, In, vAlbedo, vMRA, Nw);
...
technique11 DefaultTechnique
{
    ...
    pass Shell // 3
    {
        ...
        VertexShader = compile vs_5_0 VS_MAIN();
        GeometryShader = NULL;
        PixelShader = compile ps_5_0 PS_SHELL();
    }
}