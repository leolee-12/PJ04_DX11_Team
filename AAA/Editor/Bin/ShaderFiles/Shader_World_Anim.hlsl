...
float4x4 Build_BoneMatrix(VS_IN In)
{
...
PS_IN VS_MAIN(VS_IN In)
{
    ...
    float4x4 matBone = Build_BoneMatrix(In);
    float4 vLocalPosition = mul(float4(In.vPosition, 1.f), matBone);
    float4 vWorldPosition = mul(vLocalPosition, g_WorldMatrix);

    float4 vNormal = mul(float4(In.vNormal, 0.f), matBone);
    float4 vTangent = mul(float4(In.vTangent.xyz, 0.f), matBone);
    float4 vBinormal = mul(float4(In.vBinormal.xyz, 0.f), matBone);

    Out.vPosition = mul(mul(vWorldPosition, g_ViewMatrix), g_ProjMatrix);
    Out.vNormal = float4(normalize(mul(vNormal, g_WorldMatrix).xyz), 0.f);
    Out.vTexcoord0 = In.vTexcoord0;
    Out.vTexcoord1 = In.vTexcoord1;
    Out.vTexcoord2 = In.vTexcoord2;
    Out.vTexcoord3 = In.vTexcoord3;
    Out.vProjPos = Out.vPosition;
    Out.vTangent = float4(normalize(mul(vTangent, g_WorldMatrix).xyz), In.vTangent.w);
    Out.vBinormal = float4(normalize(mul(vBinormal, g_WorldMatrix).xyz), In.vBinormal.w);
