...
PS_OUT PS_NONEYE_WITHOUTNORMAL(PS_IN In)
{
    ...
    Out.vDiffuse = vMtrlDiffuse;
    Out.vNormal = vector(N * 0.5f + 0.5f, 0.f);
    Out.vDepth = vector(In.vProjPos.z / In.vProjPos.w, 0.f, 0.f, 0.f);
    Out.vMRA = float4(mra, 1.f);
    Out.vEmissive = float4(g_vEmissiveColor.rgb * vMtrlDiffuse.a, 1.f);
    Out.vEmissive.rgb += g_vHitFlashColor * g_fHitFlash;
...
PS_OUT PS_BLADEKNIGHT(PS_IN In)
{
    ...
    float  fMask = g_MaskTexture.Sample(LinearSampler, In.vTexcoord1).r;
    float2 HairNRG = g_NormalTexture1.Sample(LinearSampler, In.vTexcoord1).rg;
    
    nrg = lerp(nrg, HairNRG, fMask);
...
PS_OUT PS_SIRKIBBLE(PS_IN In)
{
    ...
    if (mra.r == 0.f && mra.g == 0.f && mra.b == 0.f)
        vMtrlDiffuse.rgb = mra;
...
technique11 DefaultTechnique
{
    ...
    pass NonEyeWithOutNormalPass // 4
    {
        SetRasterizerState(RS_Default);
        SetDepthStencilState(DSS_Default, 0);
        SetBlendState(BS_Default, float4(0.f, 0.f, 0.f, 0.f), 0xffffffff);
        ...
        VertexShader = compile vs_5_0 VS_MAIN();
        GeometryShader = NULL;
        PixelShader = compile ps_5_0 PS_NONEYE_WITHOUTNORMAL();
    }
    pass BladeKnight // 5
    {
        SetRasterizerState(RS_Default);
        ...
        VertexShader = compile vs_5_0 VS_MAIN();
        GeometryShader = NULL;
        PixelShader = compile ps_5_0 PS_BLADEKNIGHT();
    }
    pass PoppyFace // 6
    {
        SetRasterizerState(RS_Default);
        ...
        VertexShader = compile vs_5_0 VS_MAIN();
        GeometryShader = NULL;
        PixelShader = compile ps_5_0 PS_POPPYFACE();
    }
    pass ShadowPass // 7
    {
    ...
    pass SirKibblePass // 8
    {
        ...
        VertexShader = compile vs_5_0 VS_MAIN();
        GeometryShader = NULL;
        PixelShader = compile ps_5_0 PS_SIRKIBBLE();
    }
}