...
_bool CMesh::Ray_AABB_Ex(_fvector vOrigin, _fvector vDir, _float fPadding) const
{
    _float3 origin, dir;
    XMStoreFloat3(&origin, vOrigin);
    XMStoreFloat3(&dir, vDir);

    const _float fSafePadding = (fPadding > 0.f) ? fPadding : 0.f;

    const _float3 vMin = {
            m_vAABBMin.x - fSafePadding,
            m_vAABBMin.y - fSafePadding,
            m_vAABBMin.z - fSafePadding
    };

    const _float3 vMax = {
            m_vAABBMax.x + fSafePadding,
            m_vAABBMax.y + fSafePadding,
            m_vAABBMax.z + fSafePadding
    };
    ...
    if (fabsf(dir.x) < 1e-6f)
    {
        if (origin.x < vMin.x || origin.x > vMax.x)
            return false;
    }
    else
    {
        float t1 = (vMin.x - origin.x) / dir.x;
        float t2 = (vMax.x - origin.x) / dir.x;
        if (t1 > t2) swap(t1, t2);
        tMin = max(tMin, t1);
        tMax = min(tMax, t2);
    }
    ...
    if (fabsf(dir.y) < 1e-6f)
    {
        if (origin.y < vMin.y || origin.y > vMax.y)
            return false;
    }
    else
    {
        float t1 = (vMin.y - origin.y) / dir.y;
        float t2 = (vMax.y - origin.y) / dir.y;
        if (t1 > t2) swap(t1, t2);
        tMin = max(tMin, t1);
        tMax = min(tMax, t2);
    }
    ...
    if (fabsf(dir.z) < 1e-6f)
    {
        if (origin.z < vMin.z || origin.z > vMax.z)
            return false;
    }
    else
    {
        float t1 = (vMin.z - origin.z) / dir.z;
        float t2 = (vMax.z - origin.z) / dir.z;
        if (t1 > t2) swap(t1, t2);
        tMin = max(tMin, t1);
        tMax = min(tMax, t2);
    }
