...
void XM_CALLCONV DX::DrawCone(PrimitiveBatch<VertexPositionColor>* batch,
    FXMVECTOR origin, FXMVECTOR look,
    float radius, float halfAngle,
    FXMVECTOR color)
{
    constexpr int N = 16;
    XMVECTOR vUp = XMVectorSet(0.f, 1.f, 0.f, 0.f);
    XMVECTOR vL = XMVector3Normalize(look);

    auto rotateY = [&](float fAngle) -> XMVECTOR
        {
            return XMVector3Transform(vL, XMMatrixRotationAxis(vUp, fAngle));
        };

    XMVECTOR vLeftEnd = origin + rotateY(+halfAngle) * radius;
    XMVECTOR vRightEnd = origin + rotateY(-halfAngle) * radius;

    VertexPositionColor v0, v1;
    XMStoreFloat4(&v0.color, color);
    XMStoreFloat4(&v1.color, color);

    XMStoreFloat3(&v0.position, origin);
    XMStoreFloat3(&v1.position, vLeftEnd);
    batch->DrawLine(v0, v1);

    XMStoreFloat3(&v1.position, vRightEnd);
    batch->DrawLine(v0, v1);

    XMVECTOR vPrev = vRightEnd;
    for (int i = 1; i <= N; ++i)
    {
        float t = -halfAngle + (2.f * halfAngle) * (i / static_cast<float>(N));
        XMVECTOR vCurr = origin + rotateY(t) * radius;
        XMStoreFloat3(&v0.position, vPrev);
        XMStoreFloat3(&v1.position, vCurr);
        batch->DrawLine(v0, v1);
        vPrev = vCurr;
    }
}

void XM_CALLCONV DX::DrawRect(PrimitiveBatch<VertexPositionColor>* batch,
    FXMVECTOR origin, FXMVECTOR look,
    ...
    FXMVECTOR color)
{
    XMVECTOR vUp = XMVectorSet(0.f, 1.f, 0.f, 0.f);
    XMVECTOR vL = XMVector3Normalize(look);
    XMVECTOR vR = XMVector3Normalize(XMVector3Cross(vUp, vL));

    XMVECTOR vScaledL = vL * length;
    XMVECTOR vScaledR = vR * halfWidth;

    XMVECTOR vBL = origin - vScaledR;
    XMVECTOR vBR = origin + vScaledR;
    XMVECTOR vFL = origin + vScaledL - vScaledR;
    XMVECTOR vFR = origin + vScaledL + vScaledR;
