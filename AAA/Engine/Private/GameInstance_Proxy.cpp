...
CSound_Handle CGameInstance_Proxy::Play_SFX(const TCHAR* pSoundKey, float fVolume, ESoundBus eBus)
{
	if (!IsConnected()) 
		return CSound_Handle();
	return m_pOwner->Play_SFX(pSoundKey, fVolume, eBus);
}
CSound_Handle CGameInstance_Proxy::Play_SFX3D(const TCHAR* pSoundKey, _fvector vSoundPos, float fVolume, ESoundBus eBus)
{
	if (!IsConnected()) 
		return CSound_Handle();
	return m_pOwner->Play_SFX3D(pSoundKey, vSoundPos, fVolume, eBus);
}
CSound_Handle CGameInstance_Proxy::Play_SFX_Loop(const TCHAR* pSoundKey, float fVolume, ESoundBus eBus)
{
	if (!IsConnected()) 
		return CSound_Handle();
	return m_pOwner->Play_SFX_Loop(pSoundKey, fVolume, eBus);
}
CSound_Handle CGameInstance_Proxy::Play_SFX3D_Loop(const TCHAR* pSoundKey, _fvector vSoundPos, float fVolume, ESoundBus eBus)
{
	if (!IsConnected()) 
		return CSound_Handle();
...
void CGameInstance_Proxy::Play_BGM_Section(const TCHAR* pSoundKey, float fStart01, float fEnd01, float fVolume)
{
	if (!IsConnected()) return;
...
void CGameInstance_Proxy::Play_BGM_Fade(const TCHAR* pSoundKey, float fSeconds, float fVolume, CSound_Handle* pOut)
{
	if (!IsConnected()) 
		return;
	m_pOwner->Play_BGM_Fade(pSoundKey, fSeconds, fVolume, pOut);
}
...
HRESULT CGameInstance_Proxy::Refresh_StaticBoxPose(physx::PxRigidStatic* pActor, const _float3& vLocalCenter, _fmatrix WorldMatrix)
{
	if (!IsConnected())
		return E_FAIL;
