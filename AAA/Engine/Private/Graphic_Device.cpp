...
HRESULT CGraphic_Device::Initialize(HWND hWnd, WINMODE isWindowed, _uint iWinSizeX, _uint iWinSizeY, _Out_ ID3D11Device** ppDevice, _Out_ ID3D11DeviceContext** ppContext)
{
	...
	m_fCurWidth		= (_float)iWinSizeX;
	m_fCurHeight	= (_float)iWinSizeY;
...
HRESULT CGraphic_Device::Bind_BackBuffer()
{
	...
	ViewPortDesc.Width = m_fCurWidth;
	ViewPortDesc.Height = m_fCurHeight;
...
HRESULT CGraphic_Device::OnResize(_uint iWidth, _uint iHeight)
{
	...
	// 스왑체인 버퍼 리사이즈
	if (FAILED(m_pSwapChain->ResizeBuffers(1, iWidth, iHeight,
	...
	if (FAILED(Ready_DepthStencilView(iWidth, iHeight)))
		return E_FAIL;

	Bind_BackBuffer();

	m_fCurWidth		= (_float)iWidth;
	m_fCurHeight	= (_float)iHeight;
...
CGraphic_Device * CGraphic_Device::Create(_In_ HWND hWnd, WINMODE isWindowed, _uint iWinSizeX, _uint iWinSizeY, _Out_ ID3D11Device ** ppDevice, _Out_ ID3D11DeviceContext ** ppDeviceContextOut)
{
...
void CGraphic_Device::Free()
{
	...
	_uint iRefCnt = Safe_Release(m_pDevice);
	if (iRefCnt > 0)
	{
		wchar_t szBuf[128];
		wsprintf(szBuf, L"Device RefCount : %d\n", iRefCnt);
