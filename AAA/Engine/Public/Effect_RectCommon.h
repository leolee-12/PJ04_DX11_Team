...
{
    enum SPRITE_TIME_MODE
    {
        SPRITE_TIME_EFFECT,
        SPRITE_TIME_PARTICLE,
        SPRITE_TIME_END
    };
    ...
    ENGINE_DLL HRESULT Bind_StaticShaderValues(
        CShader* pShader, const VALUES& Values);

    ENGINE_DLL HRESULT Bind_SpriteShaderValues(
        CShader* pShader, const VALUES& Values);

    ENGINE_DLL HRESULT Bind_Roll(CShader* pShader, _float fRoll);

    ENGINE_DLL HRESULT Bind_ParticleDrawValues(
        CShader* pShader, const _float4x4& WorldMatrix,
        _float fAlpha, const _float3& vColor);

    ENGINE_DLL HRESULT Begin_AndRender(
        CShader* pShader, CVIBuffer_Rect* pBuffer, _int iPass);

    ENGINE_DLL void Update_SpriteAnimations(
        VALUES& Values, _float fRatio, _bool bEvaluateDisabled = false);

    ENGINE_DLL void Initialize_DefaultValues(VALUES& Values);

    ENGINE_DLL _int Normalize_SpriteTimeMode(_int iMode);
}
