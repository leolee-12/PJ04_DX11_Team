#ifndef Engine_Enum_h__
#define Engine_Enum_h__

namespace Engine
{
	enum class WINMODE { FULL, WIN };

	enum class PROTOTYPE { GAMEOBJECT, COMPONENT };

	enum class RENDERID { PRIORITY, SHADOW, NONBLEND, DECAL, NONLIGHT, BLEND, BLEND_HDR, OCCLUSION, SKY, DISTORTION, END };

	enum class RENDERUIID {BACK, MIDDLE, FRONT, CURTAIN, FLASH, END};

	enum class STATE { RIGHT, UP, LOOK, POSITION, END };

	enum class D3DTS { VIEW, PROJ, END };

	enum class PROJ_TYPE { PERSPEC, ORTHO, END };

	enum class LIGHT { DIRECTIONAL, POINT, SPOT, END };

	enum class MODEL { NONANIM, ANIM, MAP, ENVIRONMENT, END };

	enum class NAVI_POINT { A, B, C, END };

	enum class NAVI_LINE { AB, BC, CA, END };

	enum class COLLIDER { AABB, OBB, SPHERE, CAPSULE, TORUS, END };

	enum class DIMB { LBUTTON, RBUTTON, WHEEL, END };

	enum class DIMM { X, Y, WHEEL, END };

	enum class DEFERRED { DEBUG, DIRECTIONAL, POINT, SPOT, COMBINED, END };

	enum class POSTPROSESS {
		BRIGHT, BLUR, COMPSITE, SSAO, SSAO_BLUR, SSR,
		DOF_DOWN, DOF_BLUR, DOF_COMPOSITE, CURTAIN_COMPOSITE, OCCLUSION_SILHOUETTE,
		SPOTLIGHT_DARKEN, ESM_RESOLVE, DISTORTION_APPLY, END
	};

	enum class ANIM_EVENT_PHASE { POINT, BEGIN, TICK, END };

	enum class GVAL { FLOAT, FLOAT2, FLOAT3, FLOAT4, BOOL };
	
	enum class MTEX_TYPE { NONE, DIFFUSE, SPECULAR, AMBIENT, EMISSIVE, HEIGHT, NORMALS, SHININESS, 
		OPACITY, DISPLACEMENT, LIGHTMAP, REFLECTION, BASECOLOR, NORMALCAMERA, 
		EMISSIONCOLOR, METALNESS, ROUGHNESS, AO, UNKNOWN,
		SHEEN, CLEARCOAT, TRANSMISSION, MAYABASE, MAYASPECULAR, MAYASPECULARCOLOR, MAYASPECULARROUGHNESS,
		ANISOTROPY, GLTF_MERALLICROUGHNESS
	};

	enum class CULLING_VIEW { MAIN_CAMERA, SHADOW_DIR, EDITOR_VIEWPORT, END };

	enum class TEXT_ALIGN { LEFT, CENTER, RIGHT, END };

	enum class DEFAULT_TEXTURE { WHITE, BLACK, MAGENTA, FLAT_NORMAL, MRA, END };

	enum class ESoundBus { BGM, SFX, UI, VOICE, AMBIENT, END };
}
#endif // Engine_Enum_h__
