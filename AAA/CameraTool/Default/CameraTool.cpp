...
_uint g_iResizeWidth = { 0 };
_uint g_iResizeHeight = { 0 };
    ...
    _In_ LPWSTR    lpCmdLine, _In_ int       nCmdShow) 
{
    ...
    CGameInstance_Proxy* pGameInstance_Proxy = CGameInstance::GetProxy();

    if (FAILED(pGameInstance_Proxy->Add_Timer(TEXT("Timer_Default"))))
        return E_FAIL;
    if (FAILED(pGameInstance_Proxy->Add_Timer(TEXT("Timer_60"))))
        return E_FAIL;
    ...
    while (true)
    {
        ...
        if (!g_bAppClosing) {
            if (g_bNeedResize)
            {
                pGameInstance_Proxy->OnResize(g_iResizeWidth, g_iResizeHeight);
                g_bNeedResize = false;
            }
            ...
            {
                pGameInstance_Proxy->Compute_Timer(TEXT("Timer_60"));

                pEditorApp->Update(pGameInstance_Proxy->Get_TimeDelta(TEXT("Timer_60")), pGameInstance_Proxy->Get_RawTimeDelta(TEXT("Timer_60")));
                pEditorApp->Render();
    ...
    Safe_Release(pGameInstance_Proxy);
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
    ...
    wcex.hIcon = LoadIcon(hInstance, MAKEINTRESOURCE(IDC_CAMERATOOL));
