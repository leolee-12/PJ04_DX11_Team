...
namespace
{
	inline constexpr const _char* SLOPEBOARD_B_MODEL_PATH = "../../Resources/Map/Gimmick/Anim/SlopeBoard/SlopeBoardB.ysh";
	...
	inline constexpr const _char* SLOPEBOARD_B_ANIM_NAMES[LD_ANIM_SLOT_COUNT] = { ANIM_LANDBACK, ANIM_LANDFRONT, ANIM_LANDSTARTFRONT, "" };

	inline constexpr const _char* SLOPEBOARD_B_PLATFORM_MESH_NAME = "PlateM__BoardC";
	inline constexpr const _char* SLOPEBOARD_B_PLATFORM_BONE_NAME = "FrontAnimL";

	inline constexpr _float SLOPEBOARD_B_MIN_HALF_EXTENT = 0.01f;
	inline constexpr _float SLOPEBOARD_B_FALLEN_ANGLE_RAD = XM_PI / 3.f + 0.18f;
	inline constexpr _float SLOPEBOARD_B_FALL_VISUAL_DURATION = 1.f;
	inline constexpr _float SLOPEBOARD_B_ANIM_SPEED = 1.5f;
	...
	_matrix Make_FallenLocalMatrix(const _float3& vMin, const _float3& vMax, _float fAngleRad = SLOPEBOARD_B_FALLEN_ANGLE_RAD)
	{
...
CLD_SlopeBoardB::CLD_SlopeBoardB(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CLD_EventObject(pDevice, pContext)
{
}

CLD_SlopeBoardB::CLD_SlopeBoardB(const CLD_SlopeBoardB& Prototype)
...
HRESULT CLD_SlopeBoardB::Initialize(void* pArg)
{
	...
	{
			{ ANIM_LANDBACK, false, SLOPEBOARD_B_ANIM_SPEED },
			{ ANIM_LANDFRONT, false, SLOPEBOARD_B_ANIM_SPEED },
			{ ANIM_LANDSTARTFRONT, false, SLOPEBOARD_B_ANIM_SPEED },
	};
...
HRESULT CLD_SlopeBoardB::Validate_Initialized()
{
	...
	{
		if (m_tEventObjectDesc.strAnimNames[i] != SLOPEBOARD_B_ANIM_NAMES[i])
			return E_FAIL;
	}

	if (Find_MeshIndex_ByName(SLOPEBOARD_B_PLATFORM_MESH_NAME) < 0)
		return E_FAIL;

	if (nullptr == m_pModelCom->Get_BoneMatrixPtr(SLOPEBOARD_B_PLATFORM_BONE_NAME))
		return E_FAIL;
...
void CLD_SlopeBoardB::Update(_float fTimeDelta)
{
	...
	{
		m_fElapsed = min(m_fElapsed + fTimeDelta, SLOPEBOARD_B_FALL_VISUAL_DURATION);

		if (!m_bEndAnimationPlayed && m_fElapsed >= SLOPEBOARD_B_FALL_VISUAL_DURATION)
		{
			m_bEndAnimationPlayed = true;

			if (FAILED(Update_ToFallenPose()))
				Log_SlopeBoardPhysicsWarning("[LDSlopeBoardB] fallen platform rigid static pose update failed.");
		...
		&& m_bEndAnimationPlayed)
	{
		...
		m_fElapsed = SLOPEBOARD_B_FALL_VISUAL_DURATION;
	}
}

void CLD_SlopeBoardB::Late_Update(_float fTimeDelta)
{
	if (m_pTrigger->Is_Enabled())
	{
		const _float4x4* pBoneMatrix = m_pModelCom->Get_BoneMatrixPtr(SLOPEBOARD_B_PLATFORM_BONE_NAME);
...
void CLD_SlopeBoardB::Copy_PrototypeName(ENGINE_OBJECT_DATA* pOutData)
{
	if (nullptr == pOutData)
		return;
...
void CLD_SlopeBoardB::Register_LevelDesignSpecs()
{
	LD_SPAWN_SPEC Spec{};
	...
	{
			{ MODEL_PROTO_TAG, SLOPEBOARD_B_MODEL_PATH, MODEL::ANIM, false },
	};

	CLevelDesign_Registry::Register(Spec.strObjectName, Spec);
}

_bool CLD_SlopeBoardB::Build_Desc(const LD_OBJECT_DESC& CommonDesc, const json& jEntry, const
	LD_SPAWN_SPEC& Spec, LD_OBJECT_ENTRY* pOutEntry)
{
	...
	for (_uint i = 0; i < LD_ANIM_SLOT_COUNT; ++i)
		Desc.strAnimNames[i] = SLOPEBOARD_B_ANIM_NAMES[i];

	*pOutEntry = Desc;
	return true;
}

CGameObject* CLD_SlopeBoardB::Create_Prototype(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	return CLD_SlopeBoardB::Create(pDevice, pContext);
}

HRESULT CLD_SlopeBoardB::Ready_Components()
{
...
HRESULT CLD_SlopeBoardB::Ready_RenderComponent()
{
	m_eState = STATE::IDLE;
	m_fElapsed = 0.f;
	m_bEndAnimationPlayed = false;

	const _int iPlatformMeshIndex = Find_MeshIndex_ByName(SLOPEBOARD_B_PLATFORM_MESH_NAME);
...
HRESULT CLD_SlopeBoardB::Ready_PhysicsComponent()
{
	const _int iMeshIndex = Find_MeshIndex_ByName(SLOPEBOARD_B_PLATFORM_MESH_NAME);
	if (iMeshIndex < 0)
		return E_FAIL;
	...
	vLocalHalfExtents.x = max(vLocalHalfExtents.x, SLOPEBOARD_B_MIN_HALF_EXTENT);
	vLocalHalfExtents.y = max(vLocalHalfExtents.y, SLOPEBOARD_B_MIN_HALF_EXTENT);
	vLocalHalfExtents.z = max(vLocalHalfExtents.z, SLOPEBOARD_B_MIN_HALF_EXTENT);
...
HRESULT CLD_SlopeBoardB::Update_ToFallenPose()
{
	const _int iMeshIndex = Find_MeshIndex_ByName(SLOPEBOARD_B_PLATFORM_MESH_NAME);
	if (iMeshIndex < 0)
		return E_FAIL;
...
HRESULT CLD_SlopeBoardB::Ready_Trigger()
{
	const _int iMeshIndex = Find_MeshIndex_ByName(SLOPEBOARD_B_PLATFORM_MESH_NAME);
	if (iMeshIndex < 0)
		return E_FAIL;
...
void CLD_SlopeBoardB::SetUp_Collider_Callback()
{
	if (nullptr == m_pTrigger)
		return;
...
void CLD_SlopeBoardB::Handle_TriggerEnter(CCollider* pOther)
{
	if (nullptr == pOther)
		return;
...
_bool CLD_SlopeBoardB::Should_RenderMesh(_uint iMeshIndex) const
{
...
HRESULT CLD_SlopeBoardB::Bind_BoneMatrices(_uint iMeshIndex)
{
	if ((STATE::PLAYING != m_eState && STATE::BROKEN != m_eState)
		|| m_pModelCom->Get_MeshName(iMeshIndex) != SLOPEBOARD_B_PLATFORM_MESH_NAME)
	{
	...
	const _float fRatio = m_fElapsed / SLOPEBOARD_B_FALL_VISUAL_DURATION;
	const _float fAngleRad = SLOPEBOARD_B_FALLEN_ANGLE_RAD * EaseInQuad(fRatio);
...
CLD_SlopeBoardB* CLD_SlopeBoardB::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CLD_SlopeBoardB* pInstance = new CLD_SlopeBoardB(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CLD_SlopeBoardB");
...
CGameObject* CLD_SlopeBoardB::Clone(void* pArg)
{
	CLD_SlopeBoardB* pInstance = new CLD_SlopeBoardB(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CLD_SlopeBoardB");
		Safe_Release(pInstance);
	}
