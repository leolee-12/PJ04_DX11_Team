...
Texture2D g_MarkNormalTexture;
Texture2D g_FaceEyeTexture;
Texture2D g_MouthTexture;
Texture2D g_FaceEyeNormalTexture;
Texture2D g_CrystalTexture;
Texture2D g_HeightTexture;

float g_fCrystalGlow = 1.f;
...
static const float3 EYE_YELLOW = float3(1.f, 1.f, 0.f);
...
VS_SHADOW_OUT VS_SHADOW_NA(VS_IN_NA In)
{
    VS_SHADOW_OUT Out;
...
PS_OUT PS_WING(VS_OUT In)
{
    float3 vAlbedo, vMRA, Nw;
    Sample_PBRSet(g_DiffuseTexture, g_MRATexture, g_NormalTexture, In.vTexcoord3, In, vAlbedo, vMRA, Nw);
    return Make_GBuffer(In, vAlbedo, Nw, vMRA);
}

PS_OUT PS_SHOULDER(VS_OUT In)
{
    float3 vAlbedo, vMRA, Nw;
    Sample_PBRSet(g_DiffuseTexture, g_MRATexture, g_NormalTexture, In.vTexcoord3, In, vAlbedo, vMRA, Nw);

    float fMark = g_MarkTexture.Sample(ClampSampler, In.vTexcoord2).r;
    vAlbedo = lerp(vAlbedo, float3(1.f, 1.f, 1.f), fMark);
    
    float3 NwMark = Sample_WorldNormal(g_MarkNormalTexture, In.vTexcoord2, In);
    Nw = normalize(lerp(Nw, NwMark, fMark));
...
{
    float4 vEye = g_DiffuseTexture.Sample(ClampSampler, In.vTexcoord);
    clip(vEye.a - 0.2f);
    float3 vAlbedo = EYE_YELLOW * vEye.r;
    return Make_GBuffer(In, vAlbedo, normalize(In.vNormal.xyz), float3(0.f, 1.f, 1.f));
}

PS_OUT PS_FACE(VS_OUT In)
{
    float3 vAlbedo, vMRA, Nw;
    Sample_PBRSet(g_DiffuseTexture, g_MRATexture, g_NormalTexture, In.vTexcoord3, In, vAlbedo, vMRA, Nw);

    float4 vEye = g_FaceEyeTexture.Sample(ClampSampler, In.vTexcoord);
    vAlbedo = lerp(vAlbedo, vEye.rgb, vEye.a);
    
    float3 NwEye = Sample_WorldNormal(g_FaceEyeNormalTexture, In.vTexcoord, In);
    Nw = normalize(lerp(Nw, NwEye, vEye.a));

    float fMouth = g_MouthTexture.Sample(ClampSampler, In.vTexcoord1).r;
    vAlbedo = lerp(vAlbedo, float3(0.f, 0.f, 0.f), fMouth);

    return Make_GBuffer(In, vAlbedo, Nw, vMRA);
}

PS_OUT PS_SWORD(VS_OUT In)
{
    float3 vAlbedo, vMRA, Nw;
    Sample_PBRSet(g_DiffuseTexture, g_MRATexture, g_NormalTexture, In.vTexcoord, In, vAlbedo, vMRA, Nw);

    float fGem = (In.vTexcoord1.x > 0.15f || In.vTexcoord1.y < 0.85f) ? 1.f : 0.f;

    float3 vCrystal = g_CrystalTexture.Sample(ClampSampler, In.vTexcoord1).rgb;

    vAlbedo = lerp(vAlbedo, vCrystal, fGem);
    vMRA = lerp(vMRA, float3(0.f, 0.15f, 1.f), fGem);

    PS_OUT Out = Make_GBuffer(In, vAlbedo, Nw, vMRA);
    Out.vEmissive.rgb += vCrystal * fGem * g_fCrystalGlow;
    return Out;
}
...
technique11 DefaultTechnique
{
    ...
    pass Wing // 2
    {
        ...
        VertexShader = compile vs_5_0 VS_MAIN();
        GeometryShader = NULL;
        PixelShader = compile ps_5_0 PS_WING();
    }
    pass Shoulder // 3
    {
        SetRasterizerState(RS_Default);
        ...
        VertexShader = compile vs_5_0 VS_MAIN();
        GeometryShader = NULL;
        PixelShader = compile ps_5_0 PS_SHOULDER();
    }
    pass Eye // 4
    {
        SetRasterizerState(RS_Default);
    ...
    pass SwordShadow // 6 
    {
        ...
        SetDepthStencilState(DSS_Default, 0);
        SetBlendState(BS_Default, float4(0, 0, 0, 0), 0xffffffff);
        VertexShader = compile vs_5_0 VS_SHADOW_NA();
        GeometryShader = NULL;
        PixelShader = compile ps_5_0 PS_SHADOW();
    }
    pass Sword // 7
