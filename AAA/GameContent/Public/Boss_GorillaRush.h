...
class CBoss_GorillaRush final : public CBoss
{
    GENERATED_BODY(CBoss_GorillaRush)
public:
    static constexpr const wchar_t* PROTOTYPE_TAG = L"Proto_Boss_GorillaRush";
...
private:
    CBoss_GorillaRush(ID3D11Device*, ID3D11DeviceContext*);
    CBoss_GorillaRush(const CBoss_GorillaRush&);
    virtual ~CBoss_GorillaRush() = default;
...
public:
    ...
    virtual void On_Deserialized() override;   // 스케일 등 후처리 필요하면
...
protected:
    ...
    virtual const vector<_float>& Get_PhaseThresholds() const override { return s_Thresholds; }  // {} = 단일 페이즈
    ...
    virtual const _tchar* Get_AppearEventTag() const override { return TEXT("GorillaRush_Appear"); }
...
public:
    static CBoss_GorillaRush* Create(ID3D11Device*, ID3D11DeviceContext*);
    virtual CBoss_GorillaRush* Clone(void* pArg) override;
