...
class CLD_SlopeBoardB final : public CLD_EventObject
{
	GENERATED_BODY(CLD_SlopeBoardB)

public:
	static constexpr const _tchar* OBJECT_NAME = L"SlopeBoardB";
	static constexpr const _tchar* PROTOTYPE_TAG = L"Proto_LevelDesign_SlopeBoardB";
	static constexpr const _tchar* MODEL_PROTO_TAG = L"Proto_Component_Model_SlopeBoard_B";
...
private:
	CLD_SlopeBoardB(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	CLD_SlopeBoardB(const CLD_SlopeBoardB& Prototype);
	virtual ~CLD_SlopeBoardB() = default;
...
public:
	static CLD_SlopeBoardB* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
